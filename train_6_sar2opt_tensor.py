...
checkpoint_dir = 'ckpt_fm6_sia_tensor'
checkpoint_dir_g = 'ckpt_g6_sia_tensor'
checkpoint_file = os.path.join(checkpoint_dir, 'model.ckpt')
checkpoint_file_g = os.path.join(checkpoint_dir_g, 'model.ckpt')
train_dir='summary_fm6_sia_tensor'

def initLogging(logFilename='record_fm6_sia_tensor.log'):
  """Init for logging
  """
  logging.basicConfig(
                    level    = logging.DEBUG,
...
def gfm_test():
    ...
    with graph.as_default():
        ...
        with tf.Session() as sess:
            saver.restore(sess, tf.train.latest_checkpoint('ckpt_fm6_sia_tensor'))
...
def gfm_train():
    ...
    with graph.as_default():
        ...
        # 训练 M
        match_loss,m_output  = model.gfm_sia_tensor(g_outputs, inputs_opt, inputs_lab)
        out = tf.round(m_output)
        correct,ram = model.evaluation(out, inputs_lab)
...
#                    
                    ...
                    if step % 500 == 0 :
                        ...
                        result = result*255
                        misc.imsave('out6/_sia_tensorg_{}.png'.format(str(epoch)+"_"+str(step)), result)
